...
class Admin(commands.Cog):
    ...
    @commands.command(name="accident")
    async def areyouaccident(self, ctx: Context[BotT]):
        """ Accident? """
        if ctx.author.id == 880027559373459457:
            return await ctx.send(f"<@711443220713832528>")

        await ctx.send(f"no accidents here {ctx.author.name}")
    ...
    @commands.command(name="reloadextension")
    @commands.check(permissions.is_owner)
    ...
    @commands.command(name="directmessage")
    @commands.check(permissions.is_owner)
